...
class Dataset():
    ...
    def loadData(self, fp):
        data = []
        for l in open(fp):
            data.append(tuple(map(str, l.strip().split(',')[:3])))
        return data
        ...
        for item,rating in train[user].items():
            for sim_item, sim_value in sorted_item_sim[item][:K]:
                if sim_item not in seen_items:
                    ...
                    #     print(train[user][sim_item])
                    items[sim_item] += (rating*sim[item][sim_item])
